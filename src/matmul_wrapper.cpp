...
void matmul(const MatmulConstParam& params_c, const MatmulMutableParam& params_m, const FuseConstParams& fuse_params_c, const FuseMutableParams& fuse_params_m) {
    ...
    if (arch.avx512f) {
        ...
        xsimd::avx512f arch;
        matmulT(&arch, params_c, params_m, paramPrivate, fuse_params_m);
    }
    else if (arch.avx2) {
...
MatmulFunc getMatmulFunc(const MatmulConstParam& params_c, const FuseConstParams& fuse_params_c) {
    ...
    if (arch.avx512f) {
        const auto paramPrivate = ConvertFuseParams<float, xsimd::avx512f>(fuse_params_c);
        xsimd::avx512f arch;
        auto f = easy::jit(matmulT<float, xsimd::avx512f>, &arch, params_c, _1, paramPrivate, _2);
        return f;
    }
    else if (arch.avx2) {
        const auto paramPrivate = ConvertFuseParams<float, xsimd::avx2>(fuse_params_c);
        xsimd::avx2 arch;
        auto f = easy::jit(matmulT<float, xsimd::avx2>, &arch, params_c, _1, paramPrivate, _2,
        ...
        const auto paramPrivate = ConvertFuseParams<float, xsimd::sse4_2>(fuse_params_c);
        xsimd::sse4_2 arch;
        auto f = easy::jit(matmulT<float, xsimd::sse4_2>, &arch, params_c, _1, paramPrivate, _2);
        return f;
    }
    return MatmulFunc{nullptr};
}
}